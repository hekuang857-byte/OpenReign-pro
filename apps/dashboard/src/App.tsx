...
function AnimatedNumber({ value, suffix = '' }: { value: number; suffix?: string | number }) {
...
// ═══════════════════════════════════════════
const MOCK_TASKS = [
  { id: 'OpenReign-20260326-001', status: 'zhongshu', department: '中书省', progress: 45, time: '2分钟前', created: '2026-03-26 04:15', phase: '任务规划与拆解', totalCost: 0.05, totalTokens: 1500 },
  { id: 'OpenReign-20260326-002', status: 'doing', department: '兵部', progress: 78, time: '5分钟前', created: '2026-03-26 04:10', phase: '代码执行中', totalCost: 0.12, totalTokens: 3200 },
  { id: 'OpenReign-20260326-003', status: '已批阅', department: '户部', progress: 100, time: '10分钟前', created: '2026-03-26 04:05', phase: '已完成', totalCost: 0.08, totalTokens: 2100 },
  { id: 'OpenReign-20260326-004', status: 'menxia', department: '门下省', progress: 30, time: '15分钟前', created: '2026-03-26 04:00', phase: '审核监督中', totalCost: 0.03, totalTokens: 900 },
  { id: 'OpenReign-20260326-005', status: 'doing', department: '礼部', progress: 60, time: '20分钟前', created: '2026-03-26 03:55', phase: '文档生成中', totalCost: 0.06, totalTokens: 1800 },
];
...
function App() {
  ...
  const createTask = async () => {
    if (!newTaskContent.trim()) {
      addToast('error', '请输入任务内容');
      return;
    }
    try {
      const res = await fetch('/api/zouzhe/zhixing', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ content: newTaskContent }),
      });
      if (res.ok) {
        const data = await res.json();
        addToast('success', `奏折已呈上: ${data.taskId}`);
        setNewTaskContent('');
      ...
      try {
        ...
        if (msg.type?.startsWith('task:')) {
          ...
          if (msg.type === 'task:complete') {
            addToast('success', `任务 ${msg.taskId} 已完成`);
          } else if (msg.type === 'task:fail') {
            addToast('error', `任务 ${msg.taskId} 失败`);
          }
        }
        // 处理配置变更
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    let mf = taskFilter === 'all';
    if (!mf) {
      if (taskFilter === 'running') {
        // 审理中 = 所有非终态
        mf = !['completed', 'failed', 'cancelled', '已批阅', 'done'].includes(task.status);
      } else if (taskFilter === 'attention') {
        // 需关注 = 失败
        mf = task.status === 'failed';
      } else if (taskFilter === '已批阅') {
        // 已批阅 = 所有完成状态
        mf = ['已批阅', 'completed', 'done'].includes(task.status);
      } else {
        // 其他直接匹配状态
        mf = task.status === taskFilter;
      }
    }
    return ms && mf;
  });
  ...
  const handleRetryTask = async (taskId: string) => {
    try {
      const res = await fetch(`/api/zouzhe/${taskId}/chongshi`, { method: 'POST' });
      if (res.ok) {
        addToast('success', `任务 ${taskId} 已重试`);
        fetchTasks();
      }
    } catch (e) {
      addToast('error', '重试失败');
        ...
        <div onClick={() => setExpandedTask(isExpanded ? null : task.id)} style={{ display: 'flex', alignItems: 'center', padding: '14px 16px', cursor: 'pointer' }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px', fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>
              <span>{task.department}</span><span>{Number(task.progress || 0)}%</span><span>{task.time}</span>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '80px', height: '3px', background: 'rgba(255,255,255,0.06)', borderRadius: '2px', overflow: 'hidden' }}>
              <div style={{ width: `${Number(task.progress || 0)}%`, height: '100%', background: SC[task.status], borderRadius: '2px', transition: 'width 0.6s cubic-bezier(0.25,0.46,0.45,0.94)' }} />
            </div>
        ...
        {isExpanded && (
          <div className="animate-slide-down" style={{ padding: '0 16px 14px', borderTop: '0.5px solid rgba(255,255,255,0.04)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '12px', fontSize: '12px' }}>
              ...
              <div><span style={{ color: 'rgba(255,255,255,0.35)' }}>当前状态</span><div style={{ color: SC[task.status], marginTop: '3px' }}>{getStatusText(task.status)}</div></div>
              ...
              <div><span style={{ color: 'rgba(255,255,255,0.35)' }}>进度</span><div style={{ color: 'rgba(255,255,255,0.7)', marginTop: '3px' }}>{Number(task.progress || 0)}%</div></div>
              <div><span style={{ color: 'rgba(255,255,255,0.35)' }}>成本</span><div style={{ color: '#00A86B', marginTop: '3px' }}>${(task.totalCost || 0).toFixed(4)}</div></div>
              <div><span style={{ color: 'rgba(255,255,255,0.35)' }}>Tokens</span><div style={{ color: 'rgba(255,255,255,0.7)', marginTop: '3px' }}>{(task.totalTokens || 0).toLocaleString()}</div></div>
            </div>
            <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
              ...
              {['failed', 'cancelled'].includes(task.status) && (
                <button onClick={e => { e.stopPropagation(); handleRetryTask(task.id); }} className="apple-btn" style={{ padding: '6px 14px', borderRadius: '8px', border: 'none', background: 'rgba(10,132,255,0.12)', color: '#0A84FF', fontSize: '12px', cursor: 'pointer', fontWeight: 500 }}>重试</button>
              )}
            </div>
          </div>
        )}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#000000', color: '#fff' }}>
      ...
      {showNewTaskModal && (
        <>
          <div onClick={() => setShowNewTaskModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200 }} />
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '500px', maxWidth: '90vw', background: '#1a1a1a', borderRadius: '16px', border: '1px solid rgba(212,175,55,0.2)', padding: '24px', zIndex: 201 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ margin: 0, fontSize: '18px', color: '#D4AF37' }}>📝 新建奏折</h3>
              <button onClick={() => setShowNewTaskModal(false)} style={{ background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer' }}>✕</button>
            </div>
            <textarea
              value={newTaskContent}
              onChange={(e) => setNewTaskContent(e.target.value)}
              placeholder="请描述您的旨意..."
              style={{ width: '100%', height: '120px', background: '#2a2a2a', border: '1px solid #333', borderRadius: '8px', padding: '12px', color: '#fff', fontSize: '14px', resize: 'none', marginBottom: '16px' }}
            />
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowNewTaskModal(false)} style={{ padding: '8px 16px', background: 'transparent', border: '1px solid #333', borderRadius: '6px', color: '#888', cursor: 'pointer' }}>取消</button>
              <button onClick={createTask} style={{ padding: '8px 16px', background: '#D4AF37', border: 'none', borderRadius: '6px', color: '#0a0a0a', fontWeight: 600, cursor: 'pointer' }}>呈递奏折</button>
            </div>
          </div>
        </>
      )}
