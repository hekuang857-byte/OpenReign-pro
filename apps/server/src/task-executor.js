...
class TaskExecutor extends EventEmitter {
  constructor(config = {}) {
    ...
    // 成本追踪配置 - 从环境变量或配置读取
    this.costConfig = {
      tokenPricePer1K: config.tokenPricePer1K || parseFloat(process.env.TOKEN_PRICE_PER_1K) || 0.002,
      reportInterval: config.reportInterval || parseInt(process.env.REPORT_INTERVAL_MS) || 5 * 60 * 1000,
    };
  ...
  async execute(task) {
    ...
    // 获取当天任务序号
    const todayTasks = Array.from(this.runningTasks.values()).filter(t => {
    ...
    const taskId = task.id || `OpenReign-${dateStr}-${seq}`;
  ...
  recordCost(taskId, tokens, model = 'default') {
    ...
    // 不同模型不同价格 - 可从外部配置传入
    const modelPrices = this.config.modelPrices || {
      'gpt-4': 0.03,
      'gpt-3.5': 0.002,
      'claude': 0.008,
      'default': this.costConfig.tokenPricePer1K
    };
  ...
  // ═══════════════════════════════════════════════════════════
  // 看板 API 支持（对齐 edict 的 kanban_update.py）
  // ═══════════════════════════════════════════════════════════
  ...
  createTask(taskId, data) {
    const task = {
      ...
      startTime: Date.now(),
      lastProgressTime: Date.now(),
      flow_log: data.flow_log || [],
      progress_log: data.progressLog || [],
      todos: data.todos || []
    };
