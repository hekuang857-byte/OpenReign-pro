...
// 通用广播函数（支持两种调用方式）
function broadcast(typeOrData, data) {
  let msg;
  if (data !== undefined) {
    // 调用方式: broadcast(type, data)
    msg = JSON.stringify({ type: typeOrData, version: ++configVersion, timestamp: new Date().toISOString(), data });
  } else {
    // 调用方式: broadcast(dataObject)
    msg = JSON.stringify(typeOrData);
  }
...
// CORS 配置 - 开发环境允许 localhost，生产环境应从配置读取
const corsOptions = {
  origin: process.env.NODE_ENV === 'production' 
    ? (process.env.ALLOWED_ORIGINS?.split(',') || ['http://localhost:18790'])
    : ['http://localhost:18790', 'http://127.0.0.1:18790', 'http://localhost:5173'],
  credentials: true
};
...
function readConfig() {
  try { 
    ...
    if (err.code !== 'ENOENT') {
      console.error('[Server] 读取配置失败:', err.message);
    }
    return null; 
  }
}
...
// 部门启用/禁用（支持两种路径）
const handleToggleDept = (req, res) => {
  const { id } = req.params;
...
app.post('/api/chaoting/bumen/:id/qiyong', handleToggleDept);
app.post('/api/chaoting/bumen/:id/toggle', handleToggleDept); // 别名，兼容前端调用

// 户部 + 奏折系统 API
app.use('/api', apiRoutes);

// 看板 API - 对齐 edict 的 kanban_update.py
const { router: kanbanRouter, initKanban } = require('./routes/kanban');
app.use('/api', kanbanRouter);
// 初始化看板（传入 taskExecutor 和广播函数）
initKanban(taskExecutor, broadcast);
...
app.post('/api/zouzhe/zhixing', async (req, res) => {
  // 支持两种参数格式：{ type, description, priority } 或 { content }
  let { type, description, priority = 'normal', content } = req.body;
  
  // 如果传了 content，将其作为 description
  if (content && !description) {
    description = content;
  }
  // 如果没有传 type，默认为 general_task
  if (!type && content) {
    type = 'general_task';
  }
  
  // 生成新格式任务ID: OpenReign-YYYYMMDD-NNN
  const now = new Date();
  ...
  // 从executor获取当天任务数
  const runningTasks = taskExecutor.runningTasks || new Map();
  const todayTasks = Array.from(runningTasks.values()).filter((t) => {
    const tDate = new Date(t.startTime);
    return tDate.getFullYear() === year && 
           tDate.getMonth() === now.getMonth() && 
           tDate.getDate() === now.getDate();
  });
  const seq = String(todayTasks.length + 1).padStart(3, '0');
  const taskId = `OpenReign-${dateStr}-${seq}`;
...
// 重试任务
app.post('/api/zouzhe/:taskId/chongshi', (req, res) => {
  const { taskId } = req.params;
  const taskInfo = taskExecutor.runningTasks.get(taskId);
  ...
  // 重置任务状态
  taskInfo.status = 'taizi';
  taskInfo.retries = (taskInfo.retries || 0) + 1;
  taskInfo.flow_log.push({
    at: new Date().toISOString(),
    from: '皇上',
    to: '太子',
    remark: `重试任务（第${taskInfo.retries}次）`,
    action: 'retry'
  });
  
  // 重新执行任务
  taskExecutor.execute(taskInfo.metadata);
  
  res.json({ success: true, message: '任务已重试', retryCount: taskInfo.retries });
});
app.get('*', (req, res) => {
